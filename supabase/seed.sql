...
-- 2b) エリアと表示順（経路別実績一覧 Excel を正: 北谷 / 北部 / 那覇 / 沖縄市）。
...
update app.facilities set area_name = '北谷', display_order = v.ord
  from (values ('aquapalace',10),('aquapalace_annex',20),('yuinoie',30),
               ('tataminoyadomihama',40),('chatanhills',50)) as v(code, ord)
  where app.facilities.facility_code = v.code;

update app.facilities set area_name = '北部', display_order = v.ord
  from (values ('jyagal',60),('villakouri',70),('kondokouri',80),('imadomari',90),
               ('villayagaji',100),('Canpou',110),('poolcondyagaji',120),('rusin',130)) as v(code, ord)
  where app.facilities.facility_code = v.code;

update app.facilities set area_name = '那覇', display_order = v.ord
  from (values ('joint',140),('tataminoyadonaha',150),('elsinn_naha',160)) as v(code, ord)
  where app.facilities.facility_code = v.code;

update app.facilities set area_name = '沖縄市', display_order = 170 where facility_code = 'koza';
