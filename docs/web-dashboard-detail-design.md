...
| `status` | text | `uploaded`, `validated`, `committed`, `failed` |
...
| `source_system` | text | `minpakuin`, `neppan`, `temairazu` |
| `source_record_key` | text | ソース内一意キー |
...
| `fee_adjusted_gross_amount` | numeric | Agoda/Trip.com 補正後 |
...
create unique index on app.reservation_stay_nights (source_system, source_record_key);
```
...
- Agoda/Trip.com 補正は canonical 作成時に `fee_adjusted_gross_amount` へ保存
...
| `商品プランコード` | `source_record_key` の補助要素 |
| `更新日` | `source_record_key` の補助要素 |

変換ルール:

- `encoding = cp932`
- `reservation_key = 予約ID + "|" + 予約番号`
- `source_record_key = source_system + facility + reservation_key + 泊目 + 商品プランコード + 更新日`
...
- 初期端数処理は `tax_amount = floor(gross_amount * 10 / 110)`、`net_amount = gross_amount - tax_amount` とする。Excel差分検証で既存帳票と端数差が出る場合は `fee_adjustment_rules` または tax rule table で変更可能にする
...
| 税抜 | `fee_adjusted_gross_amount - tax_amount` |
...
| `net_amount` | `sum(fee_adjusted_gross_amount - tax_amount)` |
...
| `lead_time_total` | valid lead time の合計 |
| `lead_time_count` | valid lead time 対象予約数 |
...
canonical upsert は `source_system + source_record_key` の unique key で実施する。同じ raw ファイルを再取込しても重複しない。
...
| 予算/販売可能室数 | 初期 seed CSV で投入できるようにする |
