...
| ✅ 確定（2026-06-17 ユーザー承認） | **canonical の gross = 4要素（大人+子供+幼児+その他）を「表示の真実」とする**。parity 検証では旧ETL相当の3要素和（`gross_excl_other`）を検証ハーネス側で併走再計算し、差分主因（その他 +9予約・dropna 欠損）を可視化する。スキーマ変更なし |
...
1. ✅ **【確定 2026-06-17】その他合計額（D2）**: canonical の gross は **4要素（大人+子供+幼児+その他, 新spec準拠）を「表示の真実」**とする（ユーザー承認）。3要素和は parity 検証で併走再計算（スキーマ変更なし）。
